const visObject = {
  updateAsync: function (
    ...
    const questionSubject = queryResponse.fields.dimensions[0].name ? queryResponse.fields.dimensions[0].name : null; //questions.subject
    const questionQuestion = queryResponse.fields.dimension_like[1].name ? queryResponse.fields.dimension_like[1].name : null; //questions.question
    ...
    function visual() {
      ...
      elemEnter.on("click", function(d) {
        ...
        tooltip.html(
          questionSubject ? `<strong style="font-size: 13px; line-height: 18px">${d[questionSubject]["value"]}</strong>` : '' + 
            "<br>" + questionQuestion ? `<strong style="font-size: 11px; display: block; margin: 5px 0;">${d[questionQuestion]["value"]} ?</strong>` : '' +
    ...
    if (totalResponse && totalResponse > 19) {
      visual();
    } else {
      message();
    }
